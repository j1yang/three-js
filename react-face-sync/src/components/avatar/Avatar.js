...
export default function Model(props) {
  ...
  function BufferPoints({ count = 72 }) {
    ...
    const points = useMemo(() => {
      ...
      let count = 0;
      nodes.Wolf3D_Head.geometry.morphAttributes.position.map((v) => {
        merged = [...merged, ...v.array]
        
      })
      console.log(merged)
      return new BufferAttribute(new Float32Array(merged),3);
    }, [count]);
  
    return (
      <points scale={30} >
        <bufferGeometry>
          <bufferAttribute attach={"attributes-position"} {...points} usage={DynamicDrawUsage}/>
        </bufferGeometry>
        <pointsMaterial
          size={0.01}
          threshold={0.1}
  ...
  useEffect(()=>{
    ...
    console.log(nodes) // position: array 72 > Float32Array 6486/3=2162
